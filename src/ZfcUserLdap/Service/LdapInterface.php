...
//             $hm = $this->ldap->search($filter, $entryDN, 1, array("mail","cn","uidnumber","uid"));
			$hm = $this->ldap->search($filter, $this->active_server['baseDn'], \Zend\Ldap\Ldap::SEARCH_SCOPE_SUB,array("mail","cn","uidnumber","uid","sAMAccountName","objectGUID","memberof"));
            
//             var_dump($hm->getFirst());
//             exit();
    ...
    public function findById($id) {
        ...
        try {
            $hm = $this->ldap->search("objectGUID=$id", $this->active_server['baseDn'], \Zend\Ldap\Ldap::SEARCH_SCOPE_ONE);
            foreach ($hm as $item) {
                $this->log($item);
                return $item;
            }
